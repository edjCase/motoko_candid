...
module {
  ...
  private func encodeTypes(buffer: Buffer.Buffer<Nat8>, args: [CandidArg]) {
    let info: { compoundTypes: [CandidType]; typeCodes: [Int] } = getTypeInfo(args);
    ...
    let _ = IntX.encodeInt(buffer, info.typeCodes.size(), #signedLEB128); // Encode type count // TODO validate this is a SIGNED leb128, not unsigned
  ...
  private func encodeType(buffer: Buffer.Buffer<Nat8>, t: CandidType, typeCode: Int) {
      let _ = IntX.encodeInt(buffer, typeCode, #signedLEB128); // Encode compound type code
    switch (t) {
      case (#opt(o)) {
  ...
  private func getTypeInfo(args: [CandidArg]): { compoundTypes: [CandidType]; typeCodes: [Int] } {
    ...
    let sortedTable: [TypeInfo] = Array.sort<TypeInfo>(Iter.toArray(table.entries()), func (t1: TypeInfo, t2: TypeInfo) { Nat.compare(t1.1, t2.1) });
    let compoundTypes: [CandidType] = Iter.toArray(Iter.map<TypeInfo, CandidType>(Iter.fromArray(sortedTable), func(kv: TypeInfo) { kv.0 }));
    {
  ...
  private func addArgTypeToTable(arg: CandidArg, table: TrieMap.TrieMap<CandidType, Nat>, codes: Buffer.Buffer<Int>): CandidType {
    let (typeCodeOrIndex: Int, _type: CandidType) = switch(arg){
      ...
      case (#record(r)) {
        let fields: [RecordFieldType] = Array.map<RecordFieldArg, RecordFieldType>(r, func(f: RecordFieldArg) {
          let fieldType: CandidType = addArgTypeToTable(f.value, table, codes);
      ...
      case (#variant(v)) {
        let options: [VariantOptionType] = Array.map<RecordFieldArg, VariantOptionType>(r, func(o: VariantOptionArg) {
          let optionType: CandidType = addArgTypeToTable(o.value, table, codes);
          (o.tag, optionType);
        });
  ...
  private func getOrAdd(table: TrieMap.TrieMap<CandidType, Nat>, t: CandidType): Nat {
