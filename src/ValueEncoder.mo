...
module {
  ...
  public func encodePrincipal(buffer : Buffer.Buffer<Nat8>, value : Principal) {
    ...
    let _ = NatX.encodeNat(buffer, bytes.size(), #unsignedLEB128);
    // Encode the byte length
    for (b in Iter.fromArray(bytes)) {
      buffer.add(b);
      // Encode the raw principal bytes
    };
  };
  ...
  public func encodeVector(buffer : Buffer.Buffer<Nat8>, value : [CandidValue]) {
    let _ = NatX.encodeNat(buffer, value.size(), #unsignedLEB128);
    // Encode the length of the vector
  ...
  public func encodeRecord(
    ...
    value : [{ tag : CandidTag; value : CandidValue }],
  ) {
  ...
  public func encodeFunc(buffer : Buffer.Buffer<Nat8>, value : CandidFunc) {
    switch (value) {
      ...
      case (#transparent(t)) {
        buffer.add(1);
        // 1 if not opaque
        encodeService(buffer, t.service);
        // Encode the service
        encodeText(buffer, t.method);
        // Encode the method
      };
    };
  };
  ...
  public func encodeService(buffer : Buffer.Buffer<Nat8>, value : CandidService) {
    switch (value) {
      ...
      case (#transparent(principal)) {
        buffer.add(1);
        // 1 if not opaque
        encodePrincipal(buffer, principal);
        // Encode the service principal
      };
    };
  };
