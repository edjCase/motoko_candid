...
module {
  public func run() {
    ...
    // Principal
    test([0x44, 0x49, 0x44, 0x4C, 0x00, 0x01, 0x68, 0x01, 0x00], #principal, #principal(Principal.fromText("aaaaa-aa")));
    test([0x44, 0x49, 0x44, 0x4C, 0x00, 0x01, 0x68, 0x01, 0x1D, 0xFE, 0x97, 0x48, 0xF3, 0x57, 0x95, 0x3E, 0x20, 0xAC, 0xA9, 0x0D, 0xC2, 0x41, 0xC3, 0x37, 0x2E, 0xFE, 0x4A, 0x3A, 0xEE, 0xA0, 0xD7, 0xA9, 0xB9, 0xA1, 0x36, 0x8D, 0xE7, 0x02], #principal, #principal(Principal.fromText("g4gvl-fx6s5-epgv4-vhyqk-zkiny-ja4gn-zo7zf-dv3va-26u3t-ijwrx-tqe")));

    // Null
    test([0x44, 0x49, 0x44, 0x4C, 0x00, 0x01, 0x7F], #null_, #null_);

    // Reserved
    test([0x44, 0x49, 0x44, 0x4C, 0x00, 0x01, 0x70], #reserved, #reserved);

    // Opt
    test([0x44, 0x49, 0x44, 0x4C, 0x01, 0x6E, 0x7C, 0x01, 0x00, 0x00], #opt(#int), #opt(#null_));
      ...
      #record([
        {
          _type = #record([
            ...
            { _type = #opt(#null_); tag = #name("email") },
